...
class DocumentProcessor:
    ...
    def get_processed_forms_folder(self, doc_type):
        processed_folder = {
            "hiv certificate": "hiv_cert_form",
            "landbase certificate": "med_landbase_cert_form",
            "landbase medical exam": "med_landbase_exam_form",
            "psychological evaluation form": "psycho_eval_form",
            "seafarers certificate": "med_seafarers_cert_form",
            "seafarers medical exam": "med_seafarers_exam_form"
        }
        target = processed_folder.get(doc_type.lower())
        print(target)
        return target
    ...
    def move_processed_form(self, src_path, file_name, processed_folder):
        processed_forms_folder = os.path.join("processed_forms", processed_folder)
        
        if not os.path.exists(processed_forms_folder):
            os.makedirs(processed_forms_folder)
            
        dest_path = os.path.join(processed_forms_folder, file_name)
    ...
    def process_single_document(self, file_path, file_name):
        ...
        if response.status_code == 202:
            ...
            processed_folder = self.get_processed_forms_folder(doc_type)
            ...
            self.move_processed_form(file_path, file_name, processed_folder)
        else:
